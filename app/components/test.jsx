...
function getPeriodLimits(periodKey, offset = 0) {
    ...
    // pondělí aktuálního ISO týdne
    let monday = getIsoMonday(now);

    // posun po týdnech (pokud někdy použiješ offset)
    if (offset) {
        monday = new Date(monday);
        monday.setDate(monday.getDate() + offset * 7);
    }

    // příští týden = další pondělí
    if (periodKey === "nextWeek") {
        monday = new Date(monday);
        monday.setDate(monday.getDate() + 7);
    }

    // měsíc zůstává beze změny
    if (periodKey === "month") {
